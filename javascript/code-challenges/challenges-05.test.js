...
const newArr = people.map(x => `${x.firstName} ${ x.lastName}`);
return newArr;
};
...
const addValues = (arr) => {
const newArr = arr.reduce((accumulator, value) => accumulator + value, 0);
return newArr;
};
...
const newArr = arr.reduce((accumulator, value) => accumulator + value.purchasePrice,0);
return newArr;
};
