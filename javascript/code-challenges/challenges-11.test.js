...
const count = (target, input) => {
  const totalCount = input.reduce((count, arr) => {
    const subArrCount = arr.filter(num => num === target).length;
    return count + subArrCount;
  }, 0);
  return totalCount;
};
...
const totalSum = (input) => {
  const sum = input.reduce((total, arr) => {
    const subSum = arr.reduce((subTotal, num) => subTotal + num, 0);
    return total + subSum;
  }, 0);
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const result = input.map((arr) => {
    const filteredArr = arr.filter((num) => typeof num === 'number' && num % 5 === 0);
    const poweredArr = filteredArr.map((num) => Math.pow(2, num));
    return poweredArr;
  })
  return result;
};
...
let findMaleAndFemale = (data) => {
  const filteredData = data.filter((character) => character.gender === 'male' || character.gender === 'female');
  const characterNames = filteredData.map((character) => character.name);
  const combined = characterNames.join(' and ');
  return combined;
};
...
let findShortest = (data) => {
  const shortestCharacter = data.reduce((shortest, character) => {
    ...
    const characterHeight = parseFloat(character.height);
    if (characterHeight < shortestHeight) {
      return character;
    }
    return shortest;
  })
